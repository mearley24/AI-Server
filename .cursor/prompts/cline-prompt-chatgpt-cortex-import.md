...
# ChatGPT → Cortex Memory Import

## Context

Matt has years of ChatGPT conversation history containing decisions, ideas, research, and context about Symphony Smart Homes and the AI-Server project. This data should be ingested into Cortex (Bob's memory system at port 8102) so Bob can reference past thinking and decisions.

ChatGPT's native export (Settings → Data Controls → Export) produces a ZIP containing `conversations.json` — an array of conversation objects.
...
Messages are stored as a tree (mapping), not a flat list. To get the linear conversation, walk from `current_node` up through `parent` references, then reverse.

## Instructions

### Step 1: Create the import script

Create `tools/chatgpt_to_cortex.py`:
...
### Step 2: Create a helper script for Matt to run

Create `tools/import_chatgpt.sh` (this runs on the Mac, not in Docker):
...
### Step 3: Add redis to cortex requirements (if not already there)

Check `cortex/requirements.txt` — if `redis` is not listed, add it. The import script needs the `redis` Python package.

### Step 4: Verify

1. `python3 -c "import tools.chatgpt_to_cortex"` should not error (basic syntax check)
2. The shell script should print usage when run without args
3. Both files should be committed

### What This Enables

- Matt exports his ChatGPT history (Settings → Data Controls → Export)
- Downloads the ZIP, unzips to get `conversations.json`
- Runs `zsh tools/import_chatgpt.sh ~/Downloads/conversations.json`
- All conversations are categorized and stored in Cortex
- Bob can now reference past decisions, ideas, and context from ChatGPT sessions
- Memories tagged with `chatgpt_import` for easy filtering
- Importance weighted by conversation length and topic relevance

Commit message: `feat: add ChatGPT conversation import to Cortex — parse export, categorize, store as memories`
