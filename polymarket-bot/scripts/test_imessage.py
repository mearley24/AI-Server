...
def send_via_redis(message: str) -> bool:
    ...
    # Inside polymarket-bot container (VPN network_mode), Docker DNS is
    # unavailable so we reach Redis via host.docker.internal.
...
def send_via_http(message: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            # Inside VPN container: use host.docker.internal, then localhost
            for base in ["http://host.docker.internal:8095", "http://localhost:8095"]:
                url = f"{base}/notify"
                try:
                    req = Request(
                        url,
