...
### Cortex broken data sources fixed — 20260427T192000Z

Four Cortex dashboard data sources repaired after truth-layer audit revealed root causes. (1) **Follow-ups DB** (`sqlite3` WAL mode on read-only Docker mount): All 6 `sqlite3.connect(path)` calls updated to use `immutable=1` URI mode — bypasses WAL locking on `:ro` mounts; 4 active client follow-ups now showing. (2) **Wallet USDC** (zero balance): Bot's `/status` has USDC nested at `strategies.redeemer.usdc_balance`, not the top-level key the endpoint was reading. Fixed with deep-read; wallet now shows $3.72 USDC, MATIC gas balance, `source_type=real`. BROKEN banner no longer fires. (3) **Decisions noise** (100% D-Tools automation): Added `exclude_automation=true` (default) query param to `/api/decisions/recent` — filters `category∈{jobs,sync,heartbeat,health}` server-side; debug mode requests raw feed via `exclude_automation=false`. (4) **Activity noise** (`health.checked` + `jobs.synced` ~50% of feed): Added `debug=false` query param to `/api/activity` — server-side fetches 200 entries, filters `_ACTIVITY_NOISE_TYPES={health.checked,health.check,jobs.synced,heartbeat,tick}`, caps at 50; debug mode returns raw. Client-side `_isNoise()` also updated to filter `jobs.synced`/`heartbeat`. Debug mode wiring: `refresh()` now passes correct params in debug mode. 8 new tests; ops/tests: **1182 passed**. Verification: `ops/verification/20260427T191944Z-cortex-broken-sources-fix.md`. Still unavailable: wallet position_value (paper trades), P&L series (Redis key never pushed), Trading Intel (no UI card), Daily Digest (agent idle).


### Cortex data truth layer v4 — 20260427T230000Z

Added truth-layer classification to every critical Cortex dashboard card. New `sourceBanner(type, detail)` helper classifies data as **REAL** (green), **SIMULATED** (yellow), **OUTDATED** (gray), or **UNAVAILABLE** (red) with a visible banner at the top of each affected card. Changes: (1) **`renderWallet()`** — detects broken state (usdc=0, active=0, no snapshot_age) and shows UNAVAILABLE banner citing "Redis key portfolio:snapshot not pushed — real balance in Redeemer card"; (2) **`renderPositions()`** — upgraded from inline yellow note to full SIMULATED banner via `sourceBanner('paper', ...)` when positions are paper-only cvd_arb trades; (3) **`renderPnl()`** — SIMULATED banner when pnlSummary data present and not in debug mode, notes data is from paper simulation trades; (4) **`renderWatchdog()`** — per-service staleness classification: `_svcAge()` + `_svcClass()` compute each service's real age independently of `state=ok`; services last seen >1h show as stale/yellow, >24h as broken/red; "all clear" only shown if ALL services reported within 1h; (5) **`renderFollowups()`** — upgraded to `sourceBanner('broken', ...)` for DB unavailable state; (6) **`loadReplyInbox()`** — HTTP response status now checked; non-200 responses show UNAVAILABLE banner with status code; network errors also show UNAVAILABLE banner. 7 new tests added; ops/tests: **50 passed** (local static suite). No server-side changes — all truth layer is client-side rendering.
