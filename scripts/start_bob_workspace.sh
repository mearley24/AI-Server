...
tmux send-keys -t bob:agents.3 "watch -n 60 'curl -s http://localhost:8430/status 2>/dev/null | python3 -c \"import sys,json; d=json.load(sys.stdin)[\\\"strategies\\\"][\\\"copytrade\\\"]; print(f\\\"Pos: {d[\\\\\\\"open_positions\\\\\\\"]} | Trades: {d[\\\\\\\"daily_trades\\\\\\\"]} | Bank: \\\\\\\${d[\\\\\\\"bankroll\\\\\\\"]:.0f}\\\")\" 2>/dev/null || echo \"Bot not running\"'" Enter
