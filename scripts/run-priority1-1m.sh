...
# run-priority1-1m.sh
#
# One-command Priority 1 runner for Bob. Launches Claude Code (Sonnet 4.6 with
# 1M context) against a staged prompt that carries out all Priority 1 steps
# with verify/commit/push gating between stages.
#
# Usage: bash scripts/run-priority1-1m.sh
#
# The script itself performs ZERO risky operations. It only:
#   1. Locates the repo root (~/AI-Server or current repo).
#   2. Smoke-tests the Sonnet 4.6 [1M] model.
#   3. Writes/refreshes the staged prompt file.
#   4. Invokes Claude Code with that prompt.

set -euo pipefail

MODEL="claude-sonnet-4-6[1m]"
PROMPT_REL=".cursor/prompts/direct/priority1-stage-gate.md"

log() { printf '[run-priority1-1m] %s\n' "$*" >&2; }
die() { log "ERROR: $*"; exit 1; }

# --- 1. Resolve repo root -------------------------------------------------
resolve_repo_root() {
  # Prefer the canonical ~/AI-Server path Matt uses on Bob.
  if [ -d "${HOME}/AI-Server/.git" ]; then
    echo "${HOME}/AI-Server"
    return 0
  fi
  # Otherwise walk up from this script's location to find a git repo root.
  local here
...
REPO_ROOT="$(resolve_repo_root)" || die "could not locate AI-Server repo root"
cd "$REPO_ROOT"
log "repo root: $REPO_ROOT"

# --- 2. Verify claude CLI + Sonnet 4.6 [1M] reachable ---------------------
command -v claude >/dev/null 2>&1 || die "claude CLI not on PATH"

log "smoke-testing model: $MODEL"
SMOKE_OUT="$(claude --model "$MODEL" -p 'respond exactly: SONNET_1M_READY' 2>&1 || true)"
if ! printf '%s' "$SMOKE_OUT" | grep -q 'SONNET_1M_READY'; then
  log "smoke test output:"
  printf '%s\n' "$SMOKE_OUT" >&2
  die "Sonnet 4.6 [1M] smoke test failed — model did not return SONNET_1M_READY"
fi
log "smoke test passed"

# --- 3. Write / refresh staged prompt file --------------------------------
PROMPT_PATH="$REPO_ROOT/$PROMPT_REL"
mkdir -p "$(dirname "$PROMPT_PATH")"

log "writing staged prompt: $PROMPT_REL"

# Use a quoted heredoc so nothing inside expands on the shell side.
cat > "$PROMPT_PATH" <<'PRIORITY1_PROMPT_EOF'
...
PRIORITY1_PROMPT_EOF

log "prompt written ($(wc -c < "$PROMPT_PATH") bytes)"

# --- 4. Launch Claude Code with the staged prompt -------------------------
log "launching: claude --model $MODEL -p <staged prompt>"
